...
data_pre_func_rn <- function(df1, df2){
  df <- cbind(df1, df2)
  df3 <- df %>% pivot_longer(-c(ID, KNEE, TRIAL, tr_length,status), names_to = "time", values_to = "force")%>%
    mutate(new = paste(ID, "-",KNEE, "-",TRIAL)) 
...
ap_outlier_try <- data_pre_func_rn(ID_info, ap_grf)

ap_outlier <- subset(ap_outlier_try, ap_outlier_try$status == "Outlier")
...
p_try <- ggplot(ap_outlier_try, aes(x = t, y = force, group = new, col = new)) +
  geom_line()
p_try
```
