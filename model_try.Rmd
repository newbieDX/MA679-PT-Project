...
train.V<-ID_V_GRF_stance_N[dt.1,]
test.V<-ID_V_GRF_stance_N[-dt.1,]

train.COPx<-ID_COPx_stance[dt.1,]
test.COPx<-ID_COPx_stance[-dt.1,]

train.COPy<-ID_COPy_stance[dt.1,]
test.COPy<-ID_COPy_stance[-dt.1,]
...
try3 <- data_pre_func(train.V)
try4 <- data_pre_func(train.COPx)
try5 <- data_pre_func(train.COPy)
...
dt.2 <- sort(sample(nrow(ddd), nrow(ddd)*.7))
train.ddd<-ddd[dt.2,]
test.ddd<-ddd[-dt.2,]


FPCs.pca <- prcomp(train.ddd[, c(5:7, 17:19, 32:33, 40,45 )], center = TRUE,scale. = TRUE)
summary(FPCs.pca)
...
```{r}
par(mfrow=c(1,1))
CreateOutliersPlot(FPCAdense_AP, optns = list(K = 3, variant = 'KDE'))

par(mfrow=c(1,1))
  CreateOutliersPlot(FPCAdense_ML, optns = list(K = 3, variant = 'KDE'))  
