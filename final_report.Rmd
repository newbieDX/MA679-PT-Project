...
library(kerasR)
xtrain <- as.matrix(train.ap[c(2:101)])
x_train_lstm = array_reshape(xtrain, c(dim(xtrain)[1], 1, dim(xtrain)[2]))
x_test_lstm = array_reshape(xtest, c(dim(xtest)[1], 1, dim(xtest)[2]))

lstm_model <- keras_model_sequential() 
lstm_model %>% 
  layer_lstm(units = 64, activation = "relu", return_sequences = TRUE, input_shape = c(1,100))%>%
  layer_lstm(units = 32,activation = "relu", return_sequences = FALSE, input_shape = c(1,100))%>%
  layer_repeat_vector(1)%>%
  layer_lstm(units = 32, activation = "relu",return_sequences = TRUE, input_shape = c(1,100))%>%
  layer_lstm(units = 64,activation = "relu", return_sequences = TRUE, input_shape = c(1,100))%>%
