...
df_2 <- as.matrix(subset(ap_grf, ap_grf$status == "Good")[c(-1)])

df_1 <- as.matrix(ap_outlier[c(-1)])
color = rgb(runif(2083,0,1),runif(2083,0,1), runif(2083,0,1))


plot(df_1[1,], col = color[1], type = 'l', lwd = 1, ylim = c(-400,400))
for (i in 2:nrow(df_1)) {
  lines(df_1[i,], col = color[i], type = 'l', lwd = 1)
}

color1 = rgb(runif(13613,0,1),runif(13613,0,1), runif(13613,0,1))


plot(df_2[1,], col = color1[1], type = 'l', lwd = 1, ylim = c(-300,300))
for (i in 2:nrow(df_2)) {
  lines(df_2[i,], col = color1[i], type = 'l', lwd = 1)
}
...
ml_outlier_id <- ml_ms$outliers %>% as.data.frame()
...
dt.ap <-  sort(sample(nrow(ap_grf), nrow(ap_grf)*.7))
df_train.ap<-ap_grf[dt.ap,]
df_test.ap<-ap_grf[-dt.ap,]
...
y_train <- df_train.ap$status
y_test <- df_test.ap$status
